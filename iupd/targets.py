...
# yarrp's own -t tr_type values (see yarrp.1)
...
def resolve_target_files(path):
    """A targets path may be one file, or a directory of them (top-level
    files only - subdirectories are not scanned)."""
...
def parse_target_line(line):
    """Returns (ip, protocol, port), with port None where the protocol
    doesn't take one. Raises TargetFileError on bad input."""
...
def bucket_targets(path):
    ...
    Every malformed line is collected and reported together, rather than
    skipping bad rows silently and scanning a target list that isn't the one
