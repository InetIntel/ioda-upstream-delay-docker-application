...
# straggling replies (SHUTDOWN_WAIT in yarrp.h)
...
def run_one_scan(profile, bucket, ip_list, probe_rate, instance, cfg, cycle_start, attempt):
    """Runs yarrp for one bucket and, if it succeeded, moves the .yrp into the outbox"""
...
def run_profile(profile, buckets, cfg, cycle_start, deadline):
    """Runs one profile for this cycle, retrying failed buckets (according to config)"""
    name = profile["name"]
